...
class PersonaController extends Controller
{
    ...
    public function deleteAction(Request $request, Persona $persona)
    {
     ...
     /**
     * Muestra mayores de 20 a persona entity.
     *
     * @Route("/mayores", name="persona_delete")
     * @Method("DELETE")
     */
    public function buscarmayoresAction()
    {
        $em = $this->getDoctrine()->getManager();

        $personas = $em->getRepository('AppBundle:Persona')->buscarmayores();

        return $this->render('persona/buscar.html.twig', array(
            'personas' => $personas,
        ));
    }

     public function ApiAction($mes)
    {
         // search Songs of Frank Sinatra
        $headers = array('Accept' => 'application/json');
        $query = array('q' => 'Frank sinatra', 'type' => 'track');

        $response = Unirest\Request::get('http://api.sbif.cl/api-sbifv3/recursos_api/uf/2018/'.$mes.'?apikey=d8093171162117c0c6e8da895b00978d4e2b6a0e&formato=json',$headers,$query);
        
        
        //dump($response->body);exit(1);
        // return new response($response->body);
        //return new Response('<html><body>hola</body></html>');
      //  $json=json_decode($response, true);
      // dump($response);exit(1);
          return $this->render('persona/apiresult.html.twig', array(
            'res' => $response->body
        ));
         
     }
     
    public function twig_jscodeon_de($json)
{
